 ...
 * \ingroup TaskCtrl
 */
int vTaskDelay( portTickType xTicksToDelay ) PRIVILEGED_FUNCTION;

/**
 * task. h
 * <pre>void vTaskDelayInterruptible( portTickType xTicksToDelay );</pre>
 *
 * Delay a task for a given number of ticks or until vTaskDelayAbort is called.  
 ...
 * See the configuration section for more information.
 *
 * Apart from this additional feature of waking up pre-maturely on vTaskDelayAbort
 * all the other features and usage notes are same as function vTaskDelay described 
 * above.
 *
 * @param xTicksToDelay The amount of time, in tick periods, that
 * the calling task should block.
 * @return The remaining time left for sleep to complete as per xTicksToDelay. If
 *                 the task has slept for full xTicksToDelay then 0 is returned. If for e.g.
 *                 xTicksToDelay = 10 and thread was woken up after 6 ticks
 *                 then return value will be 4
 *
 * Example usage:
 ...
 const portTickType xDelay = 500 / portTICK_RATE_MS;
	 ...
	 for( ;; )
	 {
		 // Simply toggle the LED every 500ms, blocking between each toggle.
		 vToggleLED();
		 int leftTicks = vTaskDelayInterruptible( xDelay );
		 if(leftTicks)
		  printf ("Thread slept only for %d ticks", (xDelay - leftTicks));
	 }
 }

 * \defgroup vTaskDelayInterruptible vTaskDelayInterruptible
 * \ingroup TaskCtrl
 */
int vTaskDelayInterruptible( portTickType xTicksToDelay );

/**
 * task. h
 * <pre>void vTaskDelayAbort( xTaskHandle pxTask  );</pre>
 *
 * Abort a interruptible sleep
 *
 * INCLUDE_vTaskDelay must be defined as 1 for this function to be available.
 * See the configuration section for more information.
 *
 * The given is woken up from sleep. If it is already woken up this
 * function silently returns.
 *
 * @param pxTask Handle of the thread which is to be woken up
 *
 * \defgroup vTaskDelayAbort vTaskDelayAbort
 * \ingroup TaskCtrl
 */
void vTaskDelayAbort( xTaskHandle pxTask );
...
/**
 * task. h
 * <PRE>portTickType xTaskGetTotalTickCount( void );</PRE>
 *
 * @note Added for WMSDK
 *
 * @return The count of ticks since vTaskStartScheduler was called. Also
 *         takes wraparound into account and thus is safe to use.
 *
 * \page xTaskGetTickCount xTaskGetTotalTickCount
 * \ingroup TaskUtils
 */
unsigned long long xTaskGetTotalTickCount( void ) PRIVILEGED_FUNCTION;

/**
 * task. h
 * <PRE>portTickType xTaskGetUnblockTime( void );</PRE>
 *
 * @return The ticks till next task wakes up.
 *
 * \ingroup TaskUtils
 */
portTickType xTaskGetUnblockTime( void ) PRIVILEGED_FUNCTION;

/**
 * task. h
 * <PRE> void  xTaskUpdateTickCount (portTickType);</PRE>
 *  The count of ticks since vTaskStartScheduler was called is updated.
 *   @param [in] : ticks  :Number of ticks by which count is to be updated.
 * \ingroup TaskUtils
 */
void  xTaskUpdateTickCount(portTickType ticks ) PRIVILEGED_FUNCTION;
...
/**
 * task. h
 * <PRE>void vGetTaskNamesInList(signed char *pcWriteBuffer, int size,
 * const unsigned int *pxList) PRIVILEGED_FUNCTION;</PRE>
 *
 * Lists names of all the current tasks in a given tasklist
 *
 * @param[out] pcWriteBuffer A pointer to a buffer into which the above
 * mentioned details are copied.
 * @param[in] size Size of the input buffer pcWriteBuffer
 * @param[in] pxList Pointer to the tasklist
 *
 * \page vGetTaskNamesInList vGetTaskNamesInList
 * \ingroup TaskUtils
 */
void vGetTaskNamesInList(signed char *pcWriteBuffer, int size, const unsigned int *pxList) PRIVILEGED_FUNCTION;
...
/**
 * Reset all the run time statistics
 *
 * @note Marvell added API
 *
 * This API is provided for two reasons. One, to have a mechanism to reset
 * the counters inside TCB after the global counter overflows. Second, to
 * have a mechanism for the applications to measure load for shorter time
 * period without taking history into consideration.
 *
 */
void vTaskResetRunTimeStats();
