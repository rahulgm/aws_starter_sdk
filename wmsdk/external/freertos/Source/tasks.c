...
PRIVILEGED_DATA static portTickType xNextTaskUnblockTime						= ( portTickType ) portMAX_DELAY;
	...
	/* Marvell added function */
	static void prvResetRunTimeStatsForTasksInList( xList *pxList );
...
#define prvWakeUpDelayedTask( cancelTaskTCB )				\
{								\
	/* Check if the task is still in the list */		\
	if(cancelTaskTCB->xGenericListItem.pvContainer){	\
		/* The task is still in the list. It is still awaiting its delay to complete  */ \
		/* It is time to remove the item from the Blocked state. */ \
		vListRemove( &( cancelTaskTCB->xGenericListItem ) );	\
								\
		/* Is the task waiting on an event also? */	\
		if( cancelTaskTCB->xEventListItem.pvContainer )		\
			{					\
				vListRemove( &( cancelTaskTCB->xEventListItem ) ); \
			}					\
		prvAddTaskToReadyQueue( cancelTaskTCB );		\
	}							\
}
...
/*-----------------------------------------------------------*/
	...
	tskTCB *pxTCB;
		...
		taskENTER_CRITICAL();
		{
			...
			/* Do not wait for the idle thread to delete
			   the TCB if it is not that of the current task */
			if (pxTaskToDelete != NULL)
				prvDeleteTCB( pxTCB );
			else {
				vListInsertEnd( ( xList * )
...
/*-----------------------------------------------------------*/
	...
	int vTaskDelay( portTickType xTicksToDelay )
	{
	portTickType xTimeToWake = xTickCount;
	signed portBASE_TYPE xAlreadyYielded = pdFALSE;
		...
		/* Check if we have woken up before scheduled time */
		if ( xTimeToWake > xTickCount )
			return ( xTimeToWake - xTickCount );
		else
			return 0;
	}
	...
	{
		tskTCB *cancelTaskTCB = prvGetTCBFromHandle( pxTask );
		portDISABLE_INTERRUPTS();	
		prvWakeUpDelayedTask( cancelTaskTCB );
 ...
 *----------------------------------------------------------*/
void  xTaskUpdateTickCount(portTickType ticks) 
{
...
portTickType xTaskGetUnblockTime( void )
{
	portTickType xTicks;
	/* Critical section required if running on a 16 bit processor. */
	taskENTER_CRITICAL();
	{
		xTicks = xNextTaskUnblockTime - xTickCount;
	}
	taskEXIT_CRITICAL();
	return xTicks;
}
...
unsigned long long xTaskGetTotalTickCount( void )
{
	...
	taskENTER_CRITICAL();
	{
		xTotalTicks = ((unsigned long long)xNumOfOverflows << 32);
		xTotalTicks |= xTickCount;
	}
	taskEXIT_CRITICAL();
	return xTotalTicks;
}
	...
	unsigned portBASE_TYPE uxQueue;
		...
		vTaskSuspendAll();
		{
			...
			do
			{
				...
				if( listLIST_IS_EMPTY( &( pxReadyTasksLists[ uxQueue ] ) ) == pdFALSE )
				{
					prvResetRunTimeStatsForTasksInList( ( xList * ) &( pxReadyTasksLists[ uxQueue ] ) );
				}
			}while( uxQueue > ( unsigned short ) tskIDLE_PRIORITY );

			if( listLIST_IS_EMPTY( pxDelayedTaskList ) == pdFALSE )
			{
				prvResetRunTimeStatsForTasksInList( ( xList * ) pxDelayedTaskList );
			}

			if( listLIST_IS_EMPTY( pxOverflowDelayedTaskList ) == pdFALSE )
			{
				prvResetRunTimeStatsForTasksInList( ( xList * ) pxOverflowDelayedTaskList );
			}
			...
			#if ( INCLUDE_vTaskDelete == 1 )
			{
				if( listLIST_IS_EMPTY( &xTasksWaitingTermination ) == pdFALSE )
				{
					prvResetRunTimeStatsForTasksInList( &xTasksWaitingTermination );
				}
			}
			#endif
			...
			#if ( INCLUDE_vTaskSuspend == 1 )
			{
				if( listLIST_IS_EMPTY( &xSuspendedTaskList ) == pdFALSE )
				{
					prvResetRunTimeStatsForTasksInList( &xSuspendedTaskList );
				}
			}
			#endif
		}
...
/*-----------------------------------------------------------*/
	...
	unsigned short usStackRemaining;
		...
		do
		{
			...
			/*
			 * Add lr and pc of task to sysinfo buffer, extracted
			 * from respective tasks program stack pointer, saved
			 * automatically during context switch as 14th and
			 * 15th registers.
			 */
				sprintf( pcStatusString, ( char * ) "%s\t\t%c\t%u\t%u\t%u\t%x\t%x", pxNextTCB->pcTaskName, cStatus, ( unsigned int ) pxNextTCB->uxPriority, usStackRemaining, ( unsigned int ) pxNextTCB->uxTCBNumber, pxNextTCB->pxTopOfStack[13], pxNextTCB->pxTopOfStack[14]);
				strcat( ( char * ) pcWriteBuffer, ( char * ) pcStatusString );
				sprintf( pcStatusString, ( char * ) "\t%x\t%x\r\n", pxNextTCB->pxTopOfStack, pxNextTCB->pxStack);
			strcat( ( char * ) pcWriteBuffer, ( char * ) pcStatusString );
...
void vGetTaskNamesInList(signed char *pcWriteBuffer, int size, const unsigned int *list)
{
	xList *pxList = (xList *)list;
	tskTCB *pxTCB = NULL, *pxTCB_first;
	signed char *pcWriteBufferStart = pcWriteBuffer;
	int rem_size = size;
	int buf_len = 0;
	if (!listLIST_IS_EMPTY(pxList)) {
		listGET_OWNER_OF_NEXT_ENTRY(pxTCB, pxList);
		snprintf((char *)pcWriteBuffer, size, "%s ",
			 pxTCB->pcTaskName);
		pxTCB_first = pxTCB;
		do {
			listGET_OWNER_OF_NEXT_ENTRY(pxTCB, pxList);
			if (pxTCB != pxTCB_first) {
				if (rem_size <= 0)
					break;
					snprintf((char *)pcWriteBufferStart,
					 rem_size, "%s ", pxTCB->pcTaskName);
				buf_len = strlen((char *)pcWriteBufferStart);
				rem_size -= buf_len;
				pcWriteBufferStart += buf_len;
			}
		} while(pxTCB != pxTCB_first);
	} else {
		snprintf((char *)pcWriteBuffer, size, "%s", "-");
	}
	return;
		...
		do
		{
			...
			if( ulTotalRunTime > 0UL )
			{
				...
				if( pxNextTCB->ulRunTimeCounter == 0UL )
				{
					...
					sprintf( pcStatsString, ( char * ) "%-20s\t\t0\t\t0%%\r\n", pxNextTCB->pcTaskName );
				}
				else
				{
					...
					if( ulStatsAsPercentage > 0UL )
					{
						...
						{
							sprintf( pcStatsString, ( char * ) "%-20s\t\t%lu\t\t%lu%%\r\n", pxNextTCB->pcTaskName, pxNextTCB->ulRunTimeCounter, ulStatsAsPercentage );							
						}
						#else
						{
							/* sizeof( int ) == sizeof( long ) so a smaller
							printf() library can be used. */
							sprintf( pcStatsString, ( char * ) "%-20s\t\t%u\t\t%u%%\r\n", pxNextTCB->pcTaskName, ( unsigned int ) pxNextTCB->ulRunTimeCounter, ( unsigned int ) ulStatsAsPercentage );
						}
						#endif
					}
					else
					{
						...
						#ifdef portLU_PRINTF_SPECIFIER_REQUIRED
						{
							sprintf( pcStatsString, ( char * ) "%-20s\t\t%lu\t\t<1%%\r\n", pxNextTCB->pcTaskName, pxNextTCB->ulRunTimeCounter );							
						}
						#else
						{
							/* sizeof( int ) == sizeof( long ) so a smaller
							printf() library can be used. */
							sprintf( pcStatsString, ( char * ) "%-20s\t\t%u\t\t<1%%\r\n", pxNextTCB->pcTaskName, ( unsigned int ) pxNextTCB->ulRunTimeCounter );
						}
						#endif
					}
				}
...
/* Marvell Added Function for sake of debugging */
signed char * get_current_taskname()
{
